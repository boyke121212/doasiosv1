...
class AbsenCuti: Boyke,UITextFieldDelegate {
    ...
    private let headerLabel: UILabel = {
        let label = UILabel()
        label.text = "Ambil Foto Surat Keterangan Cuti"
    ...
    private let spPilih: UIButton = {
        // Modern UIButton.Configuration (iOS 15+)
        var config = UIButton.Configuration.plain()
        config.title = "Pilih Jenis Cuti"
        config.baseForegroundColor = .darkGray
        
        // Chevron icon - di pojok kanan
        ...
        config.imagePadding = 0  // No padding between text and edge
        
        // Content insets - text di kiri (12pt), icon akan otomatis ke kanan
        config.contentInsets = NSDirectionalEdgeInsets(top: 0, leading: 12, bottom: 0, trailing: 12)
        
        // Automatic width untuk text, chevron akan stay di kanan
        config.titleLineBreakMode = .byTruncatingTail
        
        let btn = UIButton(configuration: config)
        btn.contentHorizontalAlignment = .fill  // Changed from .leading
        ...
        // ConfigurationUpdateHandler untuk memastikan chevron selalu di kanan
        btn.configurationUpdateHandler = { button in
            var config = button.configuration ?? UIButton.Configuration.plain()
            
            // Pastikan title alignment tetap leading, tapi overall content fill
    ...
    private var jenis: String? = nil
    
    private let listJenis = [
        "Pilih Jenis Cuti",
        "Cuti Tahunan",
        "Cuti Sakit",
        "Cuti Istimewa",
    ...
    private func setupUI() {
        ...
        layoutKeterangan.addSubview(spPilih)
    ...
    private func setupConstraints() {
        ...
        spPilih.translatesAutoresizingMaskIntoConstraints = false
        ...
        NSLayoutConstraint.activate([
            ...
            spPilih.topAnchor.constraint(equalTo: labelKeterangan.bottomAnchor, constant: 8),
            spPilih.leadingAnchor.constraint(equalTo: layoutKeterangan.leadingAnchor, constant: 12),
            spPilih.trailingAnchor.constraint(equalTo: layoutKeterangan.trailingAnchor, constant: -12),
            spPilih.heightAnchor.constraint(equalToConstant: 44),
            spPilih.bottomAnchor.constraint(equalTo: layoutKeterangan.bottomAnchor, constant: -12),
    ...
    private func setupDropdown() {
        ...
        for (index, jenisItem) in listJenis.enumerated() {
            let action = UIAction(title: jenisItem) { [weak self] _ in
                guard let self = self else { return }
                
                // Update configuration (modern API)
                var config = self.spPilih.configuration ?? UIButton.Configuration.plain()
                config.title = jenisItem
                
                // Maintain chevron di pojok kanan
                let chevronConfig = UIImage.SymbolConfiguration(pointSize: 14, weight: .medium)
                config.image = UIImage(systemName: "chevron.down", withConfiguration: chevronConfig)
                config.imagePlacement = .trailing
                ...
                if index == 0 {
                    // "Pilih Jenis Cuti" - belum memilih
                    self.jenis = nil
                    config.baseForegroundColor = .darkGray
                } else {
                    self.jenis = jenisItem
                    config.baseForegroundColor = .black
                }
                
                self.spPilih.configuration = config
            }
        ...
        spPilih.menu = UIMenu(title: "", children: menuItems)
    }
    ...
    private func onKirim() {
        ...
        print("🔍 [ABSEN CUTI] Compressed file size: \(data.count) bytes")
        ...
        print("🔍 [ABSEN Cuti] Params: \(params)")
        ...
        authManager.uploadFoto(
            ...
            onSuccess: { [weak self] json in
                ...
                print("✅ [ABSEN CUTI] Upload SUCCESS!")
                print("✅ [ABSEN CUTI] Response: \(json)")
            ...
            onError: { [weak self] message in
                ...
                print("❌ [ABSEN CUTI] Upload ERROR: \(message)")
            ...
            onLogout: { [weak self] message in
                ...
                print("🔐 [ABSEN CUTI] Upload LOGOUT: \(message)")
