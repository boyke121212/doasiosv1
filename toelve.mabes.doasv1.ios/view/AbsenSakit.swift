...
class AbsenSakit: Boyke,UITextFieldDelegate {
    ...
    private let headerLabel: UILabel = {
        let label = UILabel()
        label.text = "Ambil Foto Surat Keterangan Sakit"
    ...
    private let etSakit: UITextView = {
        let tv = UITextView()
    ...
    private func setupUI() {
        ...
        layoutKeterangan.addSubview(etSakit)
    ...
    private func setupConstraints() {
        ...
        etSakit.translatesAutoresizingMaskIntoConstraints = false
        ...
        NSLayoutConstraint.activate([
            ...
            etSakit.topAnchor.constraint(equalTo: labelKeterangan.bottomAnchor, constant: 8),
            etSakit.leadingAnchor.constraint(equalTo: layoutKeterangan.leadingAnchor, constant: 12),
            etSakit.trailingAnchor.constraint(equalTo: layoutKeterangan.trailingAnchor, constant: -12),
            etSakit.heightAnchor.constraint(equalToConstant: 150),
            etSakit.bottomAnchor.constraint(equalTo: layoutKeterangan.bottomAnchor, constant: -12),
    ...
    private func onKirim() {
        let ketdin = etSakit.text.trimmingCharacters(in: .whitespacesAndNewlines)
        
        if ketdin.isEmpty {
            showToast("Masukan Keterangan Sakit dulu")
            return
        }
        
        if tanggalMulaiMillis == 0 || tanggalSelesaiMillis == 0 {
            showToast("Pilih tanggal Sakit dulu")
            return
        }
        
        guard let file = absensiManager.capturedFile,
              FileManager.default.fileExists(atPath: file.path) else {
            showToast("Ambil foto Surat Keterangan Sakit dulu")
            return
        }
        ...
        print("🔍 [ABSEN SAKIT] Compressed file size: \(data.count) bytes")
        ...
        print("🔍 [ABSEN SAKIT] Params: \(params)")
        ...
        authManager.uploadFoto(
            ...
            onSuccess: { [weak self] json in
                ...
                print("✅ [ABSEN SAKIT] Upload SUCCESS!")
                print("✅ [ABSEN SAKIT] Response: \(json)")
            ...
            onError: { [weak self] message in
                ...
                print("❌ [ABSEN SAKIT] Upload ERROR: \(message)")
            ...
            onLogout: { [weak self] message in
                ...
                print("🔐 [ABSEN SAKIT] Upload LOGOUT: \(message)")
