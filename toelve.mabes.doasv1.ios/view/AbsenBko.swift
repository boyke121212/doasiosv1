...
class AbsenBko: Boyke,UITextFieldDelegate {
    ...
    private let headerLabel: UILabel = {
        let label = UILabel()
        label.text = "Ambil Foto Surat Keterangan BKO"
    ...
    private let labelKeterangan: UILabel = {
        let label = UILabel()
        label.text = "Keterangan BKO"
        label.font = .systemFont(ofSize: 15, weight: .bold)
        label.textColor = .black
        return label
    }()
    
    private let etBko: UITextView = {
        let tv = UITextView()
    ...
    private func setupUI() {
        ...
        layoutKeterangan.addSubview(etBko)
    ...
    private func setupConstraints() {
        ...
        etBko.translatesAutoresizingMaskIntoConstraints = false
        ...
        NSLayoutConstraint.activate([
            ...
            etBko.topAnchor.constraint(equalTo: labelKeterangan.bottomAnchor, constant: 8),
            etBko.leadingAnchor.constraint(equalTo: layoutKeterangan.leadingAnchor, constant: 12),
            etBko.trailingAnchor.constraint(equalTo: layoutKeterangan.trailingAnchor, constant: -12),
            etBko.heightAnchor.constraint(equalToConstant: 150),
            etBko.bottomAnchor.constraint(equalTo: layoutKeterangan.bottomAnchor, constant: -12),
    ...
    private func onKirim() {
        let ketdin = etBko.text.trimmingCharacters(in: .whitespacesAndNewlines)
        
        if ketdin.isEmpty {
            showToast("Masukan Keterangan BKO dulu")
            return
        }
        
        if tanggalMulaiMillis == 0 || tanggalSelesaiMillis == 0 {
            showToast("Pilih tanggal BKO dulu")
            return
        }
        
        guard let file = absensiManager.capturedFile,
              FileManager.default.fileExists(atPath: file.path) else {
            showToast("Ambil foto Surat Keterangan BKO dulu")
            return
        }
        ...
        print("🔍 [ABSEN BKO] Compressed file size: \(data.count) bytes")
        ...
        authManager.uploadFoto(
            ...
            onSuccess: { [weak self] json in
                ...
                print("✅ [ABSEN BKO] Upload SUCCESS!")
                print("✅ [ABSEN BKO] Response: \(json)")
            ...
            onError: { [weak self] message in
                ...
                print("❌ [ABSEN BKO] Upload ERROR: \(message)")
            ...
            onLogout: { [weak self] message in
                ...
                print("🔐 [ABSEN BKO] Upload LOGOUT: \(message)")
